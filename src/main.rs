...
async fn main() {
    ...
    if config.init_cache_on_start {
        ...
        if let Some(mut m) = load_cache(&cache_path).await {
            ...
            maps = init_cache(&beatsaver_api, None).await;
        }

        info!("{} maps have been cached.", maps.map_metadata.len());

        let _write_res = write_cache_uncompressed(maps.encode_to_vec(), &cache_path).await;
    }

    join!(map_socket(), vote_socket());
}
